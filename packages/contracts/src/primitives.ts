...
const prefixedId = (prefix: string) =>
  z.string().regex(new RegExp(`^${prefix}_[A-Za-z0-9_-]+$`), `debe ser un identificador con prefijo "${prefix}_"`);
...
export const Handle = z.string().regex(/^[a-z0-9][a-z0-9_-]{1,29}$/, 'minúsculas, dígitos, guion y guion bajo; 2–30 caracteres');
...
export const SkillCategory = z.enum(['frontend', 'backend', 'mobile', 'data-ai', 'design', 'product', 'infra', 'other']);
...
export const ApplicationStatus = z.enum(['pending', 'accepted', 'rejected', 'withdrawn', 'auto_rejected']);
...
export const PersonRef = z.object({ id: PersonId, handle: Handle, displayName: z.string().min(1) });
export type PersonRef = z.infer<typeof PersonRef>;

export const TeamRef = z.object({ id: TeamId, name: z.string().min(1) });
export type TeamRef = z.infer<typeof TeamRef>;

export const SkillRef = z.object({ slug: z.string().min(1), label: z.string().min(1), category: SkillCategory });
export type SkillRef = z.infer<typeof SkillRef>;

export const NeedRef = SkillRef.extend({ priority: NeedPriority });
